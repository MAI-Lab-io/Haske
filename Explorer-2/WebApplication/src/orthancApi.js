...
//---------------------saving the report-----------------------------------------------------
async saveReportToOrthanc(reportBlob, orthancId) {
    const orthancUrl = `${orthancApiUrl}/studies/${orthancId}/attachments`; // Ensure you are targeting the correct URL
    try {
        const response = await axios.put(orthancUrl, reportBlob, { // Use PUT for attachments
            headers: {
                'Content-Type': 'application/octet-stream', // Set content type for binary data
                'If-Match': '*', // Optional: used to prevent overwriting if not intended
            },
        });
        console.log("Response from saving report:", response.data); // Log response
        return response; // Return the response object
    } catch (error) {
        console.error('Error saving report to Orthanc:', error);
        throw error; // Throw error for further handling
    }
},

//--------------------------Fetch report attachment from Orthanc------------------------
async getReportAttachment(orthancId, attachmentName) {
    // Correct the URL by removing any extra slashes and ensuring the correct endpoint
    const orthancUrl = `${orthancApiUrl}instances/${orthancId}/attachments/${attachmentName}/data`; 
    try {
        const response = await axios.get(orthancUrl, {
            responseType: 'blob', // Ensure we receive the response as a blob
            headers: {
                'If-None-Match': '*', // Optional: Use this if you want to check for updates
            },
        });
        return response.data; // Return the blob data
    } catch (error) {
        console.error('Error fetching report attachment:', error);
        throw error; // Throw error for further handling
    }
},
    //-----------------------------------------------------------------------------------------
    ...
    async getStudyseries(orthancId) {
        return (await axios.get(orthancApiUrl + "studies/" + orthancId + "/series" + "?requestedTags=" + store.state.configuration.requestedTagsForStudyList.join(';'))).data;
    },
    async getStudySeries(orthancId) {
